locals {
  ...
  lambda_function_name     = "${var.weather_lambda_function_name}-${var.ci_merge_request_id}"
  weather_s3_bucket_name   = "${var.weather_s3_bucket_name}-${var.ci_merge_request_id}"
}

module "weather_iam" {
  source                 = "../../modules/weather-iam"
  prefix                 = local.prefix
  weather_s3_bucket_name = local.weather_s3_bucket_name
...
# NOTE(zstyblik): no, we're not going to setup CloudFront for MRs.
# module "weather_cdn" {
